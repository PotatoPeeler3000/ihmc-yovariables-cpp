...
{
class YoLong : public YoVariable, public providers::LongProvider
{
public:
   YoLong(const std::string& name, registry::YoRegistry* registry);
   YoLong(const std::string& name, const std::string& description, registry::YoRegistry* registry);

   void increment();
   void decrement();
   void add(std::int64_t value);
   void subtract(std::int64_t value);
   bool valueEquals(std::int64_t value) const;

   std::int64_t getValue() const override;
   std::int64_t getLongValue() const;

   bool set(std::int64_t value);
   bool set(std::int64_t value, bool notifyListeners);
